...
import diffspot
import argparse
from .commands import init, register, record, execute
...
def main(argv=None):
    ...
    sub_parser = sub_parsers.add_parser('register',
        help='register content(s)')
    sub_parser.add_argument('contents', nargs='+', metavar='content',
        help='contents to add')

    sub_parser = sub_parsers.add_parser('record',
        help='record state')

    sub_parser = sub_parsers.add_parser('execute',
        help='perform an action')
    sub_parser.add_argument('action',
        help='action command')
    sub_parser.add_argument('--prerun', default=None,
        help='command to be run before action command')
    sub_parser.add_argument('--postrun', default=None,
        help='command to be run after action command')
    sub_parser.add_argument('--cwd', default=None,
...
#            help='show status of working copy')
    ...
    if args.command == 'register':
        register(args.contents)
    elif args.command == 'execute':
        execute(args.action, args.prerun, args.postrun, args.cwd)
